...
from src.utils.helpers import is_authenticate
...
def create_task(body:TaskSchema,db:Session=Depends(get_db),user=Depends(is_authenticate)):
    return controller.create_task(body,db,user)

@task_routes.get("/all_tasks",response_model=list[TaskResponseSchema],status_code=status.HTTP_200_OK)
def get_all_tasks(db:Session=Depends(get_db),user=Depends(is_authenticate)):
...
def get_one_task(task_id:int,db=Depends(get_db),user=Depends(is_authenticate)):
...
def update_task(body:TaskSchema,task_id:int, db:Session = Depends(get_db),user=Depends(is_authenticate)):
...
def delete_task(task_id:int,db:Session=Depends(get_db),user:UserModel=Depends(is_authenticate)):
